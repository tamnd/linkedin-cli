...
// `linkedin company microsoft | jq` stays clean. It waits a short beat before
// its first frame, so a command that finishes quickly never paints anything.
//
// stop() clears the line and is idempotent. A caller stops it right before the
// first record reaches stdout, so the spinner never overlaps rendered output.
type spinner struct {
	msg   string
	stopc chan struct{}
	done  chan struct{}
